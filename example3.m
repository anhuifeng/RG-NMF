...
% Step 2: Load data
filename_X = 'D:\RG-NMF\data\GSE81547_human_scaled.csv';
...
filename_Sc = fullfile("D:\RG-NMF\data\", 'GSE81547_human_Sc.csv');
dataTable_Sc = readtable(filename_Sc, 'ReadVariableNames', true, 'ReadRowNames', true);
Sc = table2array(dataTable_Sc);

filename_Sg = fullfile("D:\RG-NMF\data\", 'GSE81547_human_Sg.csv');
...
rank = 500;
sigma = 2;
gamma = 1;  % Cell graph regularization term
lambda = 1; % Gene graph regularization term
...
% Step 5: Save results
% csvwrite(fullfile(output_dir, 'RGNMF_GSE81547_human_P.csv'), P);
% csvwrite(fullfile(output_dir, 'RGNMF_GSE81547_human_Q.csv'), Q);
